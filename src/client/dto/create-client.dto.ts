...
export class CreateClientDto {
    @ApiProperty({ example: '12345678901', description: 'CPF do cliente (somente números)' })
    ...
    @ApiProperty({ example: 'john.doe@example.com', description: 'Email válido do cliente' })
    @IsEmail({}, { message: 'Email deve ter um formato válido' })
    @IsNotEmpty({ message: 'Email é obrigatório' })
    email: string;

    @ApiProperty({ example: 'password123', description: 'Senha do cliente (mínimo 6 caracteres)' })
